...
export default {
  async fetchAssetsBal(pkh) {
    ...
    try {
      ...
      for (let i = 0; i < balances.length; i++) {
        ...
        const priceUsd = new BigNumber(currentPrice).multipliedBy(new BigNumber(usdMul));
  ...
  handleChrunchBal(arr) {
    return arr.filter((val) => val.contract === process.env.VUE_APP_CONTRACTS_CRUNCH).length > 0
      ? arr.filter((val) => val.contract === process.env.VUE_APP_CONTRACTS_CRUNCH)[0].balance
      : 0;
  },

  handleCrDAOBal(arr) {
    return arr.filter((val) => val.contract === process.env.VUE_APP_CONTRACTS_CRDAO).length > 0
