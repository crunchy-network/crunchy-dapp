...
const getTezPlaceholder = (pricefeedTokens) => {
  const firstToken = pricefeedTokens.find((t) => {
    return t.pairs[0]?.sides[1]?.symbol === "XTZ";
  }).pairs[0].sides[1];
  return { ...tez, priceUsd: firstToken.usdvalue };
};
...
const buildTokenListWithoutWallet = (pricefeedTokens) => {
  ...
  return [
    getTezPlaceholder(pricefeedTokens),
    ...pricefeedTokens.map(toTokenListItem),
  ];
};
