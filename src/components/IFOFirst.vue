  ...
  <el-card class="box-card" shadow="never" style="height: 100%; width: 100%">
      <el-row style="margin: 40px;">
        <div class="img-container">
            <img class="first-project-image" :src="images[project.projectLogo]" :alt="project.projectLogo" />
        </div>
        <div class="launch-date-container">
            <span style="color: #8C8D8F; margin-right: 15px;">{{getLaunchText(project.startTime)}} </span> <span>{{ project.startTime | moment("calendar") }}</span>
        </div>
      </el-row>
  </el-card>
</template>
