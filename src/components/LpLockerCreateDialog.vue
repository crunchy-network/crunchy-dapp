<template>
  <el-dialog title="Create New LP Lock" :visible.sync="visible" width="540px" class="lock-dialog">
    <el-form v-loading.lock="loading" :model="form" ref="form" label-position="top" hide-required-asterisk>

    <div v-if="step === 1">
      <p>Search for the LP Pair you want to lock liquidity for.</p>
      <el-form-item prop="lpTokenAddress">
        <el-autocomplete
          class="el-input"
          :fetch-suggestions="queryLpTokens"
          :trigger-on-focus="false"
          v-model="form.lpTokenSearch"
          @select="handleLpTokenSelect"
          placeholder="Search for LP Token"
        >
          <template slot-scope="{ item }">
            <div style="padding: 6px 0;">
              <el-avatar shape="circle" :size="40" style="position: relative; border: 4px solid #fff; vertical-align: middle;" src="data:image/png;base64,"></el-avatar>
              <el-avatar :src="item.thumbnailUri" fit="cover" shape="circle" :size="40" style="position: relative; border: 4px solid #fff; vertical-align: middle; margin-left: -18px; margin-right: 14px;"></el-avatar>
              {{ item.value }}
            </div>
          </template>
        </el-autocomplete>
      </el-form-item>

      <div v-if="form.lpTokenAddress" style="font-weight: bold; margin-bottom: 12px;">PAIR FOUND</div>
      <div v-if="form.lpTokenAddress" class="current-balance" style="border-radius: 12px; background: #FFEECC; padding: 12px 20px; margin-bottom: 18px;">
        <el-row type="flex" align="middle" justify="space-between">
          <el-col :span="12" style="color: #303133; font-weight: bold;">
            <el-avatar shape="circle" :size="32" style="position: relative; border: 2px solid #fff; vertical-align: middle;" src="data:image/png;base64,"></el-avatar>
            <el-avatar :src="form.lpTokenThumbnailUri" fit="cover" shape="circle" :size="32" style="position: relative; border: 2px solid #fff; vertical-align: middle; margin-left: -14px; margin-right: 12px;"></el-avatar>
            {{ form.lpTokenName }}
          </el-col>
          <el-col :span="12" style="color: #8C8477; font-size: 12px; text-align: right; font-weight: bold;">BALANCE: {{ vueNumberFormat(form.lpBalance) }}</el-col>
        </el-row>
      </div>

      <el-button :disabled="!form.lpTokenAddress || form.lpBalance <= 0" @click="next" type="primary" style="border-radius: 12px; font-weight: bold; width: 100%; padding: 20px; margin-left: 0;">CONTINUE</el-button>
    </div>

    <div v-if="step === 2 && form.lpTokenAddress">
      <p>Enter the below details to proceed.</p>
      <p style="word-break: break-word;"><strong>Note:</strong> Double check parameters as the tokens can only be unlocked after the selected <strong>UNLOCK TIME</strong></p>

      <div class="current-balance" style="border-radius: 12px; background: #FFEECC; padding: 12px 20px; margin-bottom: 18px;">
        <el-row type="flex" align="middle" justify="space-between">
          <el-col :span="12" style="color: #303133; font-weight: bold;">
            <el-avatar shape="circle" :size="32" style="position: relative; border: 2px solid #fff; vertical-align: middle;" src="data:image/png;base64,"></el-avatar>
            <el-avatar :src="form.lpTokenThumbnailUri" fit="cover" shape="circle" :size="32" style="position: relative; border: 2px solid #fff; vertical-align: middle; margin-left: -14px; margin-right: 12px;"></el-avatar>
            {{ form.lpTokenName }}
          </el-col>
          <el-col :span="12" style="color: #8C8477; font-size: 12px; text-align: right; font-weight: bold;">BALANCE: {{ vueNumberFormat(form.lpBalance) }}</el-col>
        </el-row>
      </div>

      <el-row type="flex" align="top" :gutter="20">
        <el-col :span="12">
          <el-form-item
            label="Token Amount"
            prop="input"
            :rules="[
              { type: 'number', required: true, message: 'Enter an amount', transform: (v) => Number(v) },
              { type: 'number', min: 0.000001, message: 'Enter a valid amount (at least 0.000001)', transform: (v) => Number(v) }
            ]"
            style="margin-bottom: 14px;"
          >
            <el-input v-model="form.input" label="Token Amount">
              <span slot="suffix">
                <el-button type="text" size="small" style="color: #1EC37F; font-weight: bold;" @click="form.input = form.lpBalance">USE MAX</el-button>
              </span>
            </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="Unlock Time" prop="unlockTime">
            <el-date-picker
              v-model="form.unlockTime"
              type="datetime"
              placeholder="Select date and time"
              default-time="00:00:00"
            ></el-date-picker>
          </el-form-item>
        </el-col>
      </el-row>

      <el-form-item label="Service Fee Options" prop="serviceFeeId">
        <el-select v-model="form.serviceFeeId" placeholder="Select Service Fee" style="width: 100%;">
          <el-option
            v-for="item in serviceFees"
            :key="item.value"
            :label="item.label"
            :value="item.value">
          </el-option>
        </el-select>
      </el-form-item>

      <el-row type="flex" align="top" :gutter="20">
        <el-col :span="8">
          <el-button @click="prev" type="info" plain style="border-radius: 12px; font-weight: bold; width: 100%; padding: 20px; margin-left: 0;">PREVIOUS</el-button>
        </el-col>
        <el-col :span="16">
          <el-button :disabled="!form.input || !form.serviceFeeId || !form.unlockTime" @click="next" type="primary" style="border-radius: 12px; font-weight: bold; width: 100%; padding: 20px; margin-left: 0;">CONTINUE</el-button>
        </el-col>
      </el-row>
    </div>


    <div v-if="step === 3 && form.lpTokenAddress">
      <p style="word-break: break-word;"><strong>Note:</strong> Double check parameters as the tokens can only be unlocked after the selected <strong>UNLOCK TIME</strong></p>
      <h3 style="margin-top: 0;">Lock Summary</h3>
      <div class="current-balance" style="border-radius: 12px; background: #FFEECC; padding: 12px 20px; margin-bottom: 18px;">
        <el-row type="flex" align="middle" justify="space-between">
          <el-col :span="12" style="color: #303133; font-weight: bold;">
            <el-avatar shape="circle" :size="32" style="position: relative; border: 2px solid #fff; vertical-align: middle;" src="data:image/png;base64,"></el-avatar>
            <el-avatar :src="form.lpTokenThumbnailUri" fit="cover" shape="circle" :size="32" style="position: relative; border: 2px solid #fff; vertical-align: middle; margin-left: -14px; margin-right: 12px;"></el-avatar>
            {{ form.lpTokenName }}
          </el-col>
          <el-col :span="12" style="color: #8C8477; font-size: 12px; text-align: right; font-weight: bold;">BALANCE: {{ vueNumberFormat(form.lpBalance) }}</el-col>
        </el-row>
      </div>
      <el-row :gutter="20" type="flex" style="font-size: 14px; margin-bottom: 14px;">
        <el-col :span="8" style="font-weight: bold; text-align: right;">Amount to Lock:</el-col>
        <el-col :span="16">{{ vueNumberFormat(form.input * 1) }}</el-col>
      </el-row>
      <el-row :gutter="20" type="flex" style="font-size: 14px; margin-bottom: 14px;">
        <el-col :span="8" style="font-weight: bold; text-align: right;">Unlock Time:</el-col>
        <el-col :span="16">{{ form.unlockTime }}</el-col>
      </el-row>
      <el-row :gutter="20" type="flex" style="font-size: 14px; margin-bottom: 14px;">
        <el-col :span="8" style="font-weight: bold; text-align: right;">Service Fee:</el-col>
        <el-col :span="16">
          <div v-if="form.serviceFeeId === '0'">100 CRUNCH</div>
          <div v-if="form.serviceFeeId === '1'">1,000 CRUNCH</div>
          <div v-if="form.serviceFeeId === '2'">10,000 CRUNCH</div>
          <div v-if="form.serviceFeeId === '0'">{{ vueNumberFormat(form.input * 0.015) }} {{ form.lpTokenName }}</div>
          <div v-if="form.serviceFeeId === '1'">{{ vueNumberFormat(form.input * 0.010) }} {{ form.lpTokenName }}</div>
          <div v-if="form.serviceFeeId === '2'">{{ vueNumberFormat(form.input * 0.005) }} {{ form.lpTokenName }}</div>
        </el-col>
      </el-row>

      <el-alert
        v-if="form.serviceFeeId === '0'"
        :title="`Upon unlock, you will receive back ${vueNumberFormat(form.input - (form.input * 0.015))} ${form.lpTokenName} LP tokens.`"
        type="info"
        show-icon
        :closable="false"
      >
      </el-alert>
      <el-alert
        v-if="form.serviceFeeId === '1'"
        :title="`Upon unlock, you will receive back ${vueNumberFormat(form.input - (form.input * 0.010))} ${form.lpTokenName} LP tokens.`"
        type="info"
        show-icon
        :closable="false"
      >
      </el-alert>
      <el-alert
        v-if="form.serviceFeeId === '2'"
        :title="`Upon unlock, you will receive back ${vueNumberFormat(form.input - (form.input * 0.005))} ${form.lpTokenName} LP tokens.`"
        type="info"
        show-icon
        :closable="false"
      >
      </el-alert>

      <el-row type="flex" align="top" :gutter="20" style="margin-top: 14px;">
        <el-col :span="8">
          <el-button @click="prev" type="info" plain style="border-radius: 12px; font-weight: bold; width: 100%; padding: 20px; margin-left: 0;">PREVIOUS</el-button>
        </el-col>
        <el-col :span="16">
          <el-button @click="createLock" type="primary" style="border-radius: 12px; font-weight: bold; width: 100%; padding: 20px; margin-left: 0;">CREATE LOCK</el-button>
        </el-col>
      </el-row>
    </div>


    </el-form>
  </el-dialog>
</template>

<script>
import { mapState, mapActions } from 'vuex'
import ipfs from './../utils/ipfs'
import farmUtils from './../utils/farm'

export default {
  name: 'LpLockerCreateDialog',
  data() {
    return {
      form: {
        input: "",
        lpTokenSearch: "",
        lpTokenType: "",
        lpTokenAddress: "",
        lpTokenId: "",
        lpTokenName: "",
        lpTokenThumbnailUri: "",
        lpBalance: 0,
        unlockTime: "",
        serviceFeeId: ""
      },
      step: 1,
      loading: false,
      visible: false,
      serviceFees: [
        { value: '0', label: '100 CRUNCH + 1.5% of tokens' },
        { value: '1', label: '1,000 CRUNCH + 1.0% of tokens' },
        { value: '2', label: '10,000 CRUNCH + 0.5% of tokens' }
      ],
      rules: {
        unlockTime: [
          { required: true, message: 'Select an unlock date & time' }
        ],
        serviceFeeId: [
          { required: true, message: 'Select a service fee' }
        ]
      }
    }
  },
  computed: {
    ...mapState([
      'lpLockers'
    ])
  },
  methods: {

    ...mapActions([
      'createLpLock',
      'getLpBalance',
      'updateLpCurrentPrices'
    ]),

    next() {
      this.step++;
    },

    prev() {
      this.step--;
    },

    async showDialog() {
      this.step = 1;
      this.form.input = "";
      this.form.lpTokenSearch = "";
      this.form.lpTokenAddress = "";
      if (Object.prototype.hasOwnProperty.call(this.$refs, 'form')) {
        this.$refs.form.resetFields();
      }
      this.loading = true;
      this.visible = true;

      const vm = this;
      this.updateLpCurrentPrices().then(() => {
        vm.loading = false;
      });
    },

    queryLpTokens(keywords, cb) {
      let matches = [];

      // liquidity baking
      if ("liquidity baking lb".includes(keywords.toLowerCase()) || 'tzbtc'.includes(keywords.toLowerCase())) {
        matches.push({
          value: "XTZ/tzBTC (Liquidity Baking)",
          type: 'fa1',
          address: 'KT1AafHA1C1vk959wvHWBispY9Y2f3fxBUUo',
          tokenId: 0,
          thumbnailUri: "https://tzbtc.io/wp-content/uploads/2020/03/tzbtc_logo_single.svg"
        });
      }

      for (let t of this.lpLockers.priceFeed) {
        if (
          (Object.prototype.hasOwnProperty.call(t, 'name') && t.name.toLowerCase().includes(keywords.toLowerCase())) ||
          (Object.prototype.hasOwnProperty.call(t, 'symbol') && t.symbol.toLowerCase().includes(keywords.toLowerCase()))
        ) {
          t = farmUtils.overrideMetadata(t);
          if (!Object.prototype.hasOwnProperty.call(t, 'thumbnailUri')) {
            t.thumbnailUri = "https://static.thenounproject.com/png/796573-200.png";
          }
          t.thumbnailUri = ipfs.transformUri(t.thumbnailUri);
          matches.push({ value: "XTZ/" + (t.symbol || t.name), type: t.type, address: t.address, tokenId: 0, isQuipuLp: true, thumbnailUri: t.thumbnailUri });
        }
      }

      cb(matches);
    },

    handleLpTokenSelect(i) {
      this.form.lpTokenName = i.value;
      this.form.lpTokenType = i.type === 'fa2' ? 'fa2' : 'fa1';
      this.form.lpTokenAddress = i.address;
      this.form.lpTokenId = i.type === 'fa2' ? i.tokenId : '';
      this.form.lpTokenThumbnailUri = i.thumbnailUri;

      const vm = this;
      this.loading = true;
      this.getLpBalance(this.form.lpTokenAddress).then((bal) => {
        vm.form.lpBalance = bal;
        vm.loading = false;
      });
    },

    async createLock() {
      const vm = this;
      this.$refs.form.validate((valid) => {
        if (valid) {
          vm.loading = true;
          vm.createLpLock({
            amount: vm.form.input,
            serviceFeeId: vm.form.serviceFeeId,
            lockEndTime: vm.form.unlockTime,
            lpToken: {
              tokenType: vm.form.lpTokenType,
              address: vm.form.lpTokenAddress,
              tokenId: vm.form.lpTokenId || 0,
            }
          }).then(() => {
            vm.visible = false;
            vm.loading = false;
          });
        } else {
          return false;
        }
      });
    }

  }
}
</script>

<style lang="scss" scoped>
@import "../crunchy-variables.scss";
@import "~element-ui/packages/theme-chalk/src/common/var";

</style>
