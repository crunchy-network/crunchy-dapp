<template>
  <div id="nft-wallet-view" style="min-height: 50vh; display: flex; flex-direction: column;">
    <el-row type="flex" style="justify-content: flex-end; margin-bottom: 36px">
      <div>
        <el-select collapse-tags value="collections" placeholder="Collections">
          <el-option class="select" label="Collections" value="collections"> </el-option>
        </el-select>
      </div>
    </el-row>
    ...
    <el-row type="flex" align="stretch" :gutter="20">
      <el-col v-for="(nft, index) in tabledata" :key="index" :span="4">
        <nft-asset-card :art="nft.assets[0]" :count="nft.count" :icon="nft.icon" :name="nft.name" type="Collection"></nft-asset-card>
      </el-col>
    </el-row>
  ...
  methods: {
    ...
    handleVisibleData() {
      ...
      this.tabledata = this.collection.slice(
        ...
        this.nextPage * this.displayCount > this.collection.length ? this.collection.length : next * this.displayCount
      );
    },
