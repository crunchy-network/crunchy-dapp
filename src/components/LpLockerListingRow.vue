  ...
  <el-row :data-lock-id="locker.id" style="padding-bottom: 14px; font-size: 14px; font-weight: 600;" type="flex" align="top">
    <el-col :span="24">
      <div
        ...
        v-bind:style="[locker.rowExpanded ? {borderColor: '#EBEEF5'} : {borderColor: '#fff'}]"
        v-loading="locker.loading"
      >
        <el-row :gutter="20" class="locker-row" v-bind:class="{ expanded: locker.rowExpanded }" style="margin-left: 0; margin-right: 0;" type="flex" align="middle">
          <el-col :sm="2" :lg="1">
            <el-avatar v-if="locker.token.isQuipuLp" :src="logos.quipuswap" fit="cover" shape="circle" :size="40" style="background: #fff; position: relative; border: 2px solid #f6cc5b; vertical-align: middle;"></el-avatar>
          </el-col>
          <el-col :sm="8" :lg="6" style="font-weight: bold;">
            ...
            <el-avatar :src="locker.token.thumbnailUri" fit="cover" shape="circle" :size="40" style="position: relative; border: 4px solid #fff; vertical-align: middle; margin-left: -18px; margin-right: 14px;"></el-avatar>
          ...
          <el-col class="hidden-md-and-down" style="text-align: right;" :span="4" v-if="showUsd === false">{{ vueNumberFormat(locker.totalLiquidityTez) }} ꜩ</el-col>
          <el-col class="hidden-md-and-down" style="text-align: right;" :span="4" v-if="showUsd === true">{{ vueNumberFormat(locker.totalLiquidityTez * lpLockers.usdVwap, {prefix: '$', decimal: '.', thousand: ',', precision: 2}) }}</el-col>

          <el-col class="hidden-md-and-down" style="text-align: right;" :span="4">{{ vueNumberFormat(locker.amountLocked) }}</el-col>
