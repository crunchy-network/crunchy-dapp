...
export default {
  ...
  async getRkdaoBalance({ state, rootState }) {
    return tzkt.getContractBigMapKeys(
      ...
      let tokenBal = BigNumber(0);
      if (tokenLedger.data.length) {
        tokenBal = BigNumber(tokenLedger.data[0].value).div(BigNumber(10).pow(8));
      }
      return tokenBal.toNumber();
    });
  },
  ...
  async swapRckt({ state, rootState, commit, dispatch }, amountToSwap) {
    ...
    const amount = BigNumber(amountToSwap).times(BigNumber(10).pow(6)).idiv(1).toNumber();
    ...
    try {
      const batch = await getBatch()
        ...
        .withContractCall(
          swapContract.methods.swap(amount)
        )
        .withContractCall(
          rcktContract.methods.update_operators([
            {
  ...
  async swapRkdao({ state, rootState, commit, dispatch }, amountToSwap) {
    ...
    const amount = BigNumber(amountToSwap).times(BigNumber(10).pow(8)).idiv(1).toNumber();
