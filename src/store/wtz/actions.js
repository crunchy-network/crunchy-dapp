...
export default {
  ...
  async updateWtzStorage({ state }) {
    return tzkt.getContractStorage(state.contractSwap)
      .then(resp => {
        return resp.data;
      });
  },
  ...
  async loadWtzData({ state, commit, dispatch }) {
    ...
    if (!state.loading) {
      ...
      commit('updateWtzTotalTvlTez', BigNumber(storage.totalXtzSum).div(BigNumber(10).pow(6)).toNumber());
  ...
  async getWtzBalance({ state, rootState }) {
    return tzkt.getContractBigMapKeys(
      state.contractWtz,
      "ledger",
      { "key.address": rootState.wallet.pkh, active: "true" }
    ).then(tokenLedger => {
      let tokenBal = BigNumber(0);
      if (tokenLedger.data.length) {
        tokenBal = BigNumber(tokenLedger.data[0].value).div(BigNumber(10).pow(6));
      }
  ...
  async wtzWrap({ state, rootState, commit, dispatch }, amountToWrap) {
    ...
    const amount = BigNumber(amountToWrap).times(BigNumber(10).pow(6)).idiv(1).toNumber();
  ...
  async wtzUnwrap({ state, rootState, commit, dispatch }, amountToUnwrap) {
    const wtzSwap = await getWalletContract(state.contractSwap);
    const amount = BigNumber(amountToUnwrap).times(BigNumber(10).pow(6)).idiv(1).toNumber();
